...
%                         mx(n1, n, 4) = mx(n1, n, 4) + 1;  
        ...
        %   5
        if ~isempty(IDArray.item(i).getPreviousSibling.getPreviousSibling)
            siblingleftnodeName = IDArray.item(i).getPreviousSibling.getPreviousSibling.getNodeName;
            for n1 = 1:length(chunknamedic)
                if strcmp(siblingleftnodeName,chunknamedic{n1}) == 1
                    subchildnodes = IDArray.item(i).getPreviousSibling.getPreviousSibling.getChildNodes;
                    text1 = get_xml_node_content(subchildnodes);
                    rdt1 = get_rdt_from_content(childnodename, text1, chunknamelist, chunkcontentlist, chunkrdtlist);
    %                 mx(n, n1, 5) = mx(n, n1, 5) + 1;
        ...
        %   6
        if ~isempty(IDArray.item(i).getNextSibling.getNextSibling)
            siblingrightnodeName = IDArray.item(i).getNextSibling.getNextSibling.getNodeName;
            for n1 = 1:length(chunknamedic)
                if strcmp(siblingrightnodeName,chunknamedic{n1}) == 1
                    subchildnodes = IDArray.item(i).getNextSibling.getNextSibling.getChildNodes;
                    text1 = get_xml_node_content(subchildnodes);
                    rdt1 = get_rdt_from_content(childnodename, text1, chunknamelist, chunkcontentlist, chunkrdtlist);
